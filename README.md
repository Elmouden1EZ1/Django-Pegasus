...
```bash
mkvirtualenv hafida_001 -p python3.12
...
Create a database named `hafida_001`.
...
```bash
celery -A hafida_001 worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A hafida_001 worker -l INFO -B --pool=solo
```
